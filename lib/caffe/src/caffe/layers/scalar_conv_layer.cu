...
template <typename Dtype>
void ScalarConvLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  const int count = top[0]->count();
  const Dtype* bottom_data = bottom[0]->gpu_data();
  const Dtype* scalar_data = this->blobs_[0]->gpu_data();
...
template <typename Dtype>
void ScalarConvLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    Dtype* product = is_eltwise ?
        this->blobs_[0]->mutable_gpu_diff() : bottom[0]->mutable_gpu_diff();
    ...
    if (!is_eltwise) {
      ...
      if (inner_dim_ == 1) {
        ...
        Dtype* scalar_diff = this->blobs_[0]->mutable_gpu_diff();
        caffe_gpu_dot(inner_dim_, product, sum_mult, scalar_diff);
      } else {
        const Dtype* sum_mult = sum_multiplier_.gpu_data();
        sum_result = (outer_dim_ == 1) ?
            this->blobs_[0]->mutable_gpu_diff() : sum_result_.mutable_gpu_data();
...
INSTANTIATE_LAYER_GPU_FUNCS(ScalarConvLayer);
