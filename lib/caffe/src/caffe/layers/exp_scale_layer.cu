...
void ExpScaleLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ExpScaleLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    caffe_gpu_dot(count, bottom_diff, bottom_data, &scale_diff_value);
...
INSTANTIATE_LAYER_GPU_FUNCS(ExpScaleLayer);
