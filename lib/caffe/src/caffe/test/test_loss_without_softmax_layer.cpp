...
class LossWithoutSoftmaxLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  LossWithoutSoftmaxLayerTest()
      : blob_bottom_data_(new Blob<Dtype>(10, 5, 2, 3)),
        blob_bottom_label_(new Blob<Dtype>(10, 1, 2, 3)),
  ...
  virtual ~LossWithoutSoftmaxLayerTest() {
...
TYPED_TEST_CASE(LossWithoutSoftmaxLayerTest, TestDtypesAndDevices);

TYPED_TEST(LossWithoutSoftmaxLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.add_loss_weight(3);
  LossWithoutSoftmaxLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 2*1e-2, 1701, 0, 0.05);
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
      this->blob_top_vec_, 0);
}

// TYPED_TEST(LossWithoutSoftmaxLayerTest, TestForwardIgnoreLabel) {
//   typedef typename TypeParam::Dtype Dtype;
//   LayerParameter layer_param;
...
TYPED_TEST(LossWithoutSoftmaxLayerTest, TestGradientIgnoreLabel) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
...
TYPED_TEST(LossWithoutSoftmaxLayerTest, TestGradientUnnormalized) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.mutable_loss_param()->set_normalize(false);
  LossWithoutSoftmaxLayer<Dtype> layer(layer_param);
