...
struct TestSuite {
  ...
  static void TestForwardDiagOffset(
  ...
  static void TestGradientBiasGroup(const std::vector<Blob<Dtype>*>& bottom,
                                    const std::vector<Blob<Dtype>*>& top,
                                    int neighborhood_size, int num_output,
                                    int group) {
    LayerParameter layer_param;
    PermutohedralParameter* permutohedral_param =
      layer_param.mutable_permutohedral_param();
  ...
  static void TestGradientBiasColor(
      const std::vector<Blob<Dtype>*>& bottom,
      const std::vector<Blob<Dtype>*>& top,
      int neighborhood_size, int num_output, int group) {
    LayerParameter layer_param;
  ...
  static void TestGradientBiasBlurSkip(
      const std::vector<Blob<Dtype>*>& bottom,
      const std::vector<Blob<Dtype>*>& top,
      int neighborhood_size, int num_output, int group) {
    LayerParameter layer_param;
 ...
 private:
  static
  Dtype kernel(const std::vector<Dtype>& left,
  ...
  static
  void BruteForce(const int num_output, const int group,
                  const std::vector<Blob<Dtype>*>& bottom,
      ...
      for (int i = 0; i < in_count; ++i) {
        for (int j = 0; j < out_count; ++j) {
          ...
          for (int f = 0; f < feature_size; ++f) {
            in_f[f] = in_features[(n * feature_size + f)
                                  * in_count + i];
            out_f[f] = out_features[(n * feature_size + f)
                                    * out_count + j];
          }
        ...
        for (int i = 0; i < in_count; ++i) {
          for (int j = 0; j < out_count; ++j) {
            ...
            for (int c = g * in_group_size;
                 c < std::min<int>((g + 1) * in_group_size,
                                   channel);
                 ++c) {
              ...
              for (int o = g * out_group_size;
                   o < std::min<int>((g + 1) * out_group_size,
                                     num_output);
                   ++o) {
  ...
  static
  void CheckBruteForce(const int num_output, const int group,
                       const std::vector<Blob<Dtype>*>& bottom,
                       const std::vector<Blob<Dtype>*>& top) {
...
#define DEFINE_TYPED_TEST(test, type, neighborhood_size, num_output,    \
                          group, bottom)                                \
  TYPED_TEST(PermutohedralLayerTest,                                    \
...
DEFINE_TYPED_TEST(TestSetup, Permutohedral,
                  3, 1, 1, bottom_)
DEFINE_TYPED_TEST(TestSetup, GaussPermutohedral,
                  2, 2, 2, big_bottom_)
DEFINE_TYPED_TEST(TestForward, Permutohedral,
                  3, 1, 1, big_bottom_)
DEFINE_TYPED_TEST(TestForward, Permutohedral,
                  3, 2, 1, big_bottom_)
DEFINE_TYPED_TEST(TestForward, Permutohedral,
                  3, 2, 2, big_bottom_)
DEFINE_TYPED_TEST(TestForward, GaussPermutohedral,
                  2, 2, 2, big_bottom_)
DEFINE_TYPED_TEST(TestForward, Permutohedral,
                  3, 1, 1, big_color_bottom_)
DEFINE_TYPED_TEST(TestForward, Permutohedral,
                  3, 2, 1, big_color_bottom_)
DEFINE_TYPED_TEST(TestForward, Permutohedral,
                  3, 2, 2, big_color_bottom_)
DEFINE_TYPED_TEST(TestForward, GaussPermutohedral,
                  2, 2, 2, big_color_bottom_)
DEFINE_TYPED_TEST(TestForwardDiagOffset, Permutohedral,
                  2, 2, 1, big_color_bottom_)
...
DEFINE_TYPED_TEST(TestGradient, Permutohedral,
                  3, 2, 2, bottom_)
DEFINE_TYPED_TEST(TestGradient, GaussPermutohedral,
                  2, 2, 2, bottom_)
DEFINE_TYPED_TEST(TestGradientBias, Permutohedral,
                  3, 1, 1, bottom_)
DEFINE_TYPED_TEST(TestGradientBias, Permutohedral,
                  3, 2, 1, bottom_)
DEFINE_TYPED_TEST(TestGradientBias, Permutohedral,
...
DEFINE_TYPED_TEST(TestGradientBias, Permutohedral,
                  3, 3, 2, bottom_)
DEFINE_TYPED_TEST(TestGradientBias, GaussPermutohedral,
                  2, 2, 2, bottom_)

DEFINE_TYPED_TEST(TestGradientBlurSkip, Permutohedral,
                  0, 2, 1, bottom_)
DEFINE_TYPED_TEST(TestGradientBlurSkip, Permutohedral,
                  0, 2, 2, bottom_)
DEFINE_TYPED_TEST(TestGradientBiasBlurSkip, Permutohedral,
                  0, 2, 1, bottom_)
DEFINE_TYPED_TEST(TestGradientBiasBlurSkip, Permutohedral,
                  0, 2, 2, bottom_)

DEFINE_TYPED_TEST(TestGradientBiasColor, Permutohedral,
                  2, 1, 1, color_bottom_)
DEFINE_TYPED_TEST(TestGradientBiasGroup, Permutohedral,
                  2, 2, 2, color_bottom_)
DEFINE_TYPED_TEST(TestGradientBiasGroup, GaussPermutohedral,
                  2, 2, 2, color_bottom_)
